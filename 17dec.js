...
async function cryptoCurrency({coin,curr}) {
  ...
  const response = await fetch(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=${curr}&ids=${coin}`);
...
async function weatherInformation({city}) {
    const response = await fetch(`http://api.weatherapi.com/v1/current.json?key=f8f0447e041e464393f154518251712&q=${city}&aqi=no`);
    const data = await response.json();
...
const cryptoInfo = {
    name:"cryptoCurrency",
    description: "You give the current price or other information related to cryptocurrency like bitcoin and ethereum etc",
    parameters: {
        type: Type.OBJECT,
        properties: {
            coin:{
            ...
            curr:{
                type:Type.STRING,
                description: "It will be the name of currency like inr , usd etc",
            }
        },
        required:['coin','curr']
    }
